...
async function loadEpisode(episodeIndex) {
  ...
  if (el.newsLayer) el.newsLayer.classList.add("hidden");
  if (el.dialogueArea) el.dialogueArea.classList.remove("hidden");
  if (el.newsText) el.newsText.textContent = "";
...
async function playBlackFlash(keep = false) {
...
if (token !== lineRenderToken) return;
  ...
  if (isNews) {
    if (el.dialogueArea) el.dialogueArea.classList.add("hidden");
    if (el.newsLayer) el.newsLayer.classList.remove("hidden");
    if (el.newsText) {
      const newsText = Array.isArray(line.text) ? line.text.join("\n") : (line.text || "");
      el.newsText.textContent = newsText;
    }
    el.nameMain.textContent = "";
    el.nameSub.textContent = "";
